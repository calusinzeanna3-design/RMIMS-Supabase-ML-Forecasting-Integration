...
async function handleSaveReceive() {
    ...
    if (currentReceiveMode === "package") {
        ...
        // Prepend to receipts & re-render instantly (< 20ms)
        ...
        toast(`Received ${pkgCount} package(s) of ${currentReceiveProduct.name} (${matItems.length} ingredients restocked)`, "success");
        closeReceiveModal();

        // 3. Local sync event broadcast
        try {
        ...
        if (window.RMIMS_NOTIFICATIONS?.addNotification) {
            window.RMIMS_NOTIFICATIONS.addNotification({
                id: `notif-rec-pkg-${Date.now()}`,
                category: 'receiving',
                priority: 'success',
                title: 'Package Restocked',
...
function openMaterialBreakdownModal(mat) {
    ...
    state.disbursements.forEach(d => {
        if (String(d.material_id) === String(mat.id)) {
            ...
            const pName = (d.finished_product_name || d.activity_type || "").trim() || "General Usage";
            if (!isGenericOperationalName(pName)) {
                prodDisbMap.set(pName, (prodDisbMap.get(pName) || 0) + qty);
            }
        }
    });
    ...
    associatedProds.forEach(p => {
        if (!prodDisbMap.has(p.name)) {
            prodDisbMap.set(p.name, 0);
        }
    ...
    if (tbody) {
        const allKeys = new Set([...associatedProds.map(p => p.name), ...prodDisbMap.keys(), ...prodRecMap.keys()]);

        if (allKeys.size === 0) {
            tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 20px; color: var(--rm-ink-dim);">No finished products or movements mapped to this material.</td></tr>`;
        } else {
            ...
            tbody.innerHTML = sortedKeys.map(key => {
                ...
                const disb = prodDisbMap.get(key) || 0;
                return `
                    <tr>
...
function initEventListeners() {
    ...
    // 2. Card 1 Filters (Product Search, Sort & Page Size)
    ...
    // 3. Card 1 Filters (Material Search, Sort & Page Size)
    ...
    // Package Stepper - Disburse Modal
    ...
    // Supabase Realtime Channel Subscription for live cross-user updates
    if (supabase && typeof supabase.channel === "function" && !window.__rmimsMaUserChannel) {
        window.__rmimsMaUserChannel = supabase
            .channel("rmims_user_material_activity_sync")
